...
export class CompanySearcherService {
   ...
   */
  public async getCompanies(pageSize: number, search: SearchFields): Promise<ShortCompanyResultPage> {
    var response = await(fetch(search.toUrl(CompanySearcherService.API_URL+"/company",pageSize)));
